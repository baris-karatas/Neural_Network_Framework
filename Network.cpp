...
void Network::feed_forwad() {
    //multiply_mat(_inputs,hidden_layer.at(0).get_weights());
    //Activation_fun_sigmo(multiply_mat(_inputs,hidden_layer.at(0).get_weights()));
    ...
    for(size_t i=0;i<layer_num-1;i++){
        ...
        //hidden_layer.at(i+1).set_input_values(nodes);
    ...
    //Output::set_input_values(outputs);
...
std::vector<float> Network::find_errors_hidden(std::vector<std::vector<float>>&weights,std::vector<double>&error_l,std::vector<double>&error_r,std::vector<float>&output_l,std::vector<float>&output_r){
    ...
    for(size_t i=0;i<right_node_num;i++){
        /*for(size_t j=0;j<left_node_num;j++){
            //mp_weight+=(weights.at(j).at(i));
            //sum_epl+=weights.at(j).at(i)*output_l.at(j);
        }*/
        //sum_epl=(1/(1+exp(sum_epl*(-1))));
        //sum_epl=(sum_epl)*(1-sum_epl);
        for(size_t j=0;j<left_node_num;j++){
            //fuc=weights.at(j).at(i)/tmp_weight;
            //fuc=(fuc)*error_r.at(i);
            weigths_ds.at(j).at(i)=error_r.at(i)*(output_r.at(i)*(1-output_r.at(i)))*weights.at(j).at(i);
            //hidden_errors.at(j)+=error_r.at(i)*(output_r.at(i)*(1-output_r.at(i)))*weights.at(j).at(i);
...
/*
void Network::find_last_hidden_errors() {
    float tmp_weight;
    std::vector<float>hidden_errors(hidden_node_num,0.0);
    std::vector<std::vector<float>>weigths_ds(hidden_node_num,std::vector<float>(output_node_num,0));
    for(size_t i=0;i<output_node_num;i++){
        for(size_t j=0;j<hidden_node_num;j++){
            tmp_weight+=Output::get_output_weights().at(j).at(i);
        }
        for(size_t j=0;j<hidden_node_num;j++){
            weigths_ds.at(j).at(i)=(Output::get_output_weights().at(j).at(i)/tmp_weight)*Output::get_error_values().at(i);
        }
        tmp_weight=0.0;
    }
    ...
    hidden_layer.at(hidden_layer.size()-1).set_error_values(hidden_errors);
}*/
...
void Network::write_nodes_weights() {
   ...
   // weigts.open("_weights",std::ios::out);
    if(!weigts)
